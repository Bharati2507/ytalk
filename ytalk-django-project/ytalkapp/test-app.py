...
if not os.environ.get('OPENAI_API_KEY'):
    os.environ['OPENAI_API_KEY'] = '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'
...
# key = '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' #@param {type:"string"}
...
def post_link():
    print("I am here ")
    f = True
    link = 'https://www.youtube.com/watch?v=mPbAwFGjnko&ab_channel=FreedominThought' #@param {type:"string"}
    # file_path = 'AztecPodcast.mp4' #@param {type:"string"}
    # file_path="/Users/bharati/coriolis_work/hackathon23/django-project/ytalk/ytalkapp/mp4files/test.mp4"
    ...
    if link:
        ...
        print(f"title: {title}")
        # extracted_text = """"heloooooooo """
        
        # output, extracted_text, res  = transcribe_me(file_path)
        # print(f"output: {output}")
        # print(f"extracted_text: {extracted_text}")
        # print(f"res: {res}")
        # print(f"{type(output)}: {type(extracted_text)} : {type(extracted_text)} ")
        vtt_path = f"{dir_path}/vttfiles/{slugify(title)}.vtt" #os.path.join(".", f"{slugify(title)}.vtt")
        print(f"vtt_path: {vtt_path}")
        ...
        # question = "What are the first five principles in Musashi's Dokodō as they relate to different areas of study such as math, logic, or computer science, and how can they be applied to the concept of exploring and looking for evidence of the truth until the feeling of doubt is resolved?"
